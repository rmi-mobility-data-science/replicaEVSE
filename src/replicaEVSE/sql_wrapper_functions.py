...
def add_data_from_csv(filepath, database, table, pw, dtype_list, weekday, add_weekday):
    connection_replica = create_db_connection("localhost", "root", pw, 'replica')
    ...
    with open(filepath, 'r') as read_obj:
        ...
        for row in csv_reader:
            ...
            if i > 0:
                #print(row)
                data = tuple(['NULL' if row[x[1]]=='\\N' else x[0](row[x[1]]) for x in zip(dtype_list,list(range(0,len(dtype_list))))])
                data = str(data)
                data = data.replace("'NULL'","NULL")
                # if 'NULL' in data:
                #     print(data)
                ...
                INSERT INTO """ + table + """ VALUES
                """ + data + ';'
                execute_query(connection_replica, populate)
            i+=1